  ...
  return (
    <div className="todo-list">
      ...
      {todos.map((todo, index) => (
        <TodoItem key={index} id={`todo-item-${index}`} className={`todo-item ${todo.isCompleted ? 'completed' : ''}`}>
          <input type="checkbox" checked={todo.isCompleted} readOnly />
          <span>{todo.text}</span>
          {todo.isCompleted ? <FaTrophy className="trophy-icon" /> : <Hourglass size={24} style={{ marginLeft: '170px' }} />}
        </TodoItem>
      ))}
    </div>
  );
